...
#include "gpio_api.h"
#include "rtc_api.h"
#include "tECNLElectricallyOperatedGate_tecsgen.h"
...
struct electrically_operated_gate_t electrically_operated_gate_class_data = {
};
...
int open/close_setting_prop_set (const EPRPINIB *item, const void *src, int size, bool_t *anno)
{
    ...
    *anno = *((uint8_t*)item->exinf) != *((uint8_t*)src);
    switch (*(uint8_t*)src) {
      case 0x41: tECNLElectricallyOperatedGate_cElectricallyOperatedGate_setOpenCloseSetting_Open( );
        break;
      case 0x42: tECNLElectricallyOperatedGate_cElectricallyOperatedGate_setOpenCloseSetting_Close( );
        break;
      case 0x43: tECNLElectricallyOperatedGate_cElectricallyOperatedGate_setOpenCloseSetting_Stop( );
        break;
default:
        return 0;
	}return 1;
...
int onoff_prop_set(const EPRPINIB *item, const void *src, int size, bool_t *anno)
{
	...
	*anno = *((uint8_t*)item->exinf) != *((uint8_t*)src);
	switch(*(uint8_t *)src){
	case 0x30: tECNLElectricallyOperatedGate_cElectricallyOperatedGate_setOperatingStatus_ON( );
		break;
	case 0x31: tECNLElectricallyOperatedGate_cElectricallyOperatedGate_setOperatingStatus_OFF( );
		break;
	default:
		return 0;
	}return 1;
...
int alarm_prop_set(const EPRPINIB *item, const void *src, int size, bool_t *anno)
{
	...
	*anno = *((uint8_t *)item->exinf) != *((uint8_t *)src);
	switch(*(uint8_t *)src){
	case 0x41: tECNLElectricallyOperatedGate_cElectricallyOperatedGate_setFaultStatus_Fault( );
	case 0x42: tECNLElectricallyOperatedGate_cElectricallyOperatedGate_setFaultStatus_NoFault( );
