...
#include "gpio_api.h"
#include "rtc_api.h"
#include "tECNLSmartElectricEnergyMeter_tecsgen.h"
...
struct smart_electric_energy_meter_t smart_electric_energy_meter_class_data = {
};
...
int onoff_prop_set(const EPRPINIB *item, const void *src, int size, bool_t *anno)
{
	...
	*anno = *((uint8_t*)item->exinf) != *((uint8_t*)src);
	switch(*(uint8_t *)src){
	case 0x30: tECNLSmartElectricEnergyMeter_cSmartElectricEnergyMeter_setOperatingStatus_ON( );
		break;
	case 0x31: tECNLSmartElectricEnergyMeter_cSmartElectricEnergyMeter_setOperatingStatus_OFF( );
		break;
	default:
		return 0;
	}return 1;
...
int alarm_prop_set(const EPRPINIB *item, const void *src, int size, bool_t *anno)
{
	...
	*anno = *((uint8_t *)item->exinf) != *((uint8_t *)src);
	switch(*(uint8_t *)src){
	case 0x41: tECNLSmartElectricEnergyMeter_cSmartElectricEnergyMeter_setFaultStatus_Fault( );
	case 0x42: tECNLSmartElectricEnergyMeter_cSmartElectricEnergyMeter_setFaultStatus_NoFault( );
