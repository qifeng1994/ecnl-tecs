...
#include "gpio_api.h"
#include "rtc_api.h"
#include "tECNLFlameSensor_tecsgen.h"
...
struct flame_sensor_t flame_sensor_class_data = {
};
...
int onoff_prop_set(const EPRPINIB *item, const void *src, int size, bool_t *anno)
{
	...
	*anno = *((uint8_t*)item->exinf) != *((uint8_t*)src);
	switch(*(uint8_t *)src){
	case 0x30: tECNLFlameSensor_cFlameSensor_setOperatingStatus_ON( );
		break;
	case 0x31: tECNLFlameSensor_cFlameSensor_setOperatingStatus_OFF( );
		break;
	default:
		return 0;
	}return 1;
...
int alarm_prop_set(const EPRPINIB *item, const void *src, int size, bool_t *anno)
{
	...
	*anno = *((uint8_t *)item->exinf) != *((uint8_t *)src);
	switch(*(uint8_t *)src){
	case 0x41: tECNLFlameSensor_cFlameSensor_setFaultStatus_Fault( );
	case 0x42: tECNLFlameSensor_cFlameSensor_setFaultStatus_NoFault( );
