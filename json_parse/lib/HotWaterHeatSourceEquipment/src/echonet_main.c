...
#include "gpio_api.h"
#include "rtc_api.h"
#include "tECNLHotWaterHeatSourceEquipment_tecsgen.h"
...
struct hot_water_heat_source_equipment_t hot_water_heat_source_equipment_class_data = {
};
...
int onoff_prop_set(const EPRPINIB *item, const void *src, int size, bool_t *anno)
{
	...
	*anno = *((uint8_t*)item->exinf) != *((uint8_t*)src);
	switch(*(uint8_t *)src){
	case 0x30: tECNLHotWaterHeatSourceEquipment_cHotWaterHeatSourceEquipment_setOperatingStatus_ON( );
		break;
	case 0x31: tECNLHotWaterHeatSourceEquipment_cHotWaterHeatSourceEquipment_setOperatingStatus_OFF( );
		break;
	default:
		return 0;
	}return 1;
...
int alarm_prop_set(const EPRPINIB *item, const void *src, int size, bool_t *anno)
{
	...
	*anno = *((uint8_t *)item->exinf) != *((uint8_t *)src);
	switch(*(uint8_t *)src){
	case 0x41: tECNLHotWaterHeatSourceEquipment_cHotWaterHeatSourceEquipment_setFaultStatus_Fault( );
	case 0x42: tECNLHotWaterHeatSourceEquipment_cHotWaterHeatSourceEquipment_setFaultStatus_NoFault( );
