...
#include "gpio_api.h"
#include "rtc_api.h"
#include "tECNLRangeHood_tecsgen.h"
...
struct range_hood_t range_hood_class_data = {
};
struct node_profile_object_t local_node_data = {
};
int ventilation_air_flow_rate_setting_prop_set (const EPRPINIB *item, const void *src, int size, bool_t *anno)
{
    ...
    *anno = *((uint8_t*)item->exinf) != *((uint8_t*)src);
    switch (*(uint8_t*)src) {
      case 0x41: tECNLRangeHood_cRangeHood_setVentilationAirFlowRateSetting_Automatic( );
        break;
default:
        return 0;
	}return 1;
...
int onoff_prop_set(const EPRPINIB *item, const void *src, int size, bool_t *anno)
{
	...
	*anno = *((uint8_t*)item->exinf) != *((uint8_t*)src);
	switch(*(uint8_t *)src){
	case 0x30: tECNLRangeHood_cRangeHood_setOperatingStatus_ON( );
		break;
	case 0x31: tECNLRangeHood_cRangeHood_setOperatingStatus_OFF( );
		break;
	default:
		return 0;
	}return 1;
...
int alarm_prop_set(const EPRPINIB *item, const void *src, int size, bool_t *anno)
{
	...
	*anno = *((uint8_t *)item->exinf) != *((uint8_t *)src);
	switch(*(uint8_t *)src){
	case 0x41: tECNLRangeHood_cRangeHood_setFaultStatus_Fault( );
	case 0x42: tECNLRangeHood_cRangeHood_setFaultStatus_NoFault( );
