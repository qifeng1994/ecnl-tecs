...
#include "gpio_api.h"
#include "rtc_api.h"
#include "tECNLCombinationMicrowaveOven_tecsgen.h"
...
struct combination_microwave_oven_t combination_microwave_oven_class_data = {
};
...
int onoff_prop_set(const EPRPINIB *item, const void *src, int size, bool_t *anno)
{
	...
	*anno = *((uint8_t*)item->exinf) != *((uint8_t*)src);
	switch(*(uint8_t *)src){
	case 0x30: tECNLCombinationMicrowaveOven_cCombinationMicrowaveOven_setOperatingStatus_ON( );
		break;
	case 0x31: tECNLCombinationMicrowaveOven_cCombinationMicrowaveOven_setOperatingStatus_OFF( );
		break;
	default:
		return 0;
	}return 1;
...
int alarm_prop_set(const EPRPINIB *item, const void *src, int size, bool_t *anno)
{
	...
	*anno = *((uint8_t *)item->exinf) != *((uint8_t *)src);
	switch(*(uint8_t *)src){
	case 0x41: tECNLCombinationMicrowaveOven_cCombinationMicrowaveOven_setFaultStatus_Fault( );
	case 0x42: tECNLCombinationMicrowaveOven_cCombinationMicrowaveOven_setFaultStatus_NoFault( );
