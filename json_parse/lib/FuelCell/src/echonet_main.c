...
void designated_power_generation_status_prop_set (const EPRPINIB *item, const void *src, int size, bool_t *anno)
{
    ...
    *anno = *((uint8_t*)item->exinf) != *((uint8_t*)src);
    switch (*(uint8_t*)src) {
      case 0x41: cFuelCell_setPowerGenerationAtTheMax.Rating( );
        break;
      case 0x42: cFuelCell_setLoadFollowingPowerGeneration( );
        break;
default:
        return 0;
}
