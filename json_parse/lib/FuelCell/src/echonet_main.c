...
#include "gpio_api.h"
#include "rtc_api.h"
#include "tECNLFuelCell_tecsgen.h"
...
struct fuel_cell_t fuel_cell_class_data = {
};
struct node_profile_object_t local_node_data = {
};
int designated_power_generation_status_prop_set (const EPRPINIB *item, const void *src, int size, bool_t *anno)
{
    ...
    *anno = *((uint8_t*)item->exinf) != *((uint8_t*)src);
    switch (*(uint8_t*)src) {
      case 0x41: tECNLFuelCell_cFuelCell_setDesignatedPowerGenerationStatus_PowerGenerationAtTheMax.Rating( );
        break;
      case 0x42: tECNLFuelCell_cFuelCell_setDesignatedPowerGenerationStatus_LoadFollowingPowerGeneration( );
        break;
default:
        return 0;
	}return 1;
...
int onoff_prop_set(const EPRPINIB *item, const void *src, int size, bool_t *anno)
{
	...
	*anno = *((uint8_t*)item->exinf) != *((uint8_t*)src);
	switch(*(uint8_t *)src){
	case 0x30: tECNLFuelCell_cFuelCell_setOperatingStatus_ON( );
		break;
	case 0x31: tECNLFuelCell_cFuelCell_setOperatingStatus_OFF( );
		break;
	default:
		return 0;
	}return 1;
...
int alarm_prop_set(const EPRPINIB *item, const void *src, int size, bool_t *anno)
{
	...
	*anno = *((uint8_t *)item->exinf) != *((uint8_t *)src);
	switch(*(uint8_t *)src){
	case 0x41: tECNLFuelCell_cFuelCell_setFaultStatus_Fault( );
	case 0x42: tECNLFuelCell_cFuelCell_setFaultStatus_NoFault( );
