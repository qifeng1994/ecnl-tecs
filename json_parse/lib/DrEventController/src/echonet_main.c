...
#include "gpio_api.h"
#include "rtc_api.h"
#include "tECNLDrEventController_tecsgen.h"
...
struct dr_event_controller_t dr_event_controller_class_data = {
};
struct node_profile_object_t local_node_data = {
};
int notificationIdDesignation_prop_set (const EPRPINIB *item, const void *src, int size, bool_t *anno)
{
...
int onoff_prop_set(const EPRPINIB *item, const void *src, int size, bool_t *anno)
{
	...
	*anno = *((uint8_t*)item->exinf) != *((uint8_t*)src);
	switch(*(uint8_t *)src){
	case 0x30: tECNLDrEventController_cDrEventController_setOperatingStatus_ON( );
		break;
	case 0x31: tECNLDrEventController_cDrEventController_setOperatingStatus_OFF( );
		break;
	default:
		return 0;
	}return 1;
...
int alarm_prop_set(const EPRPINIB *item, const void *src, int size, bool_t *anno)
{
	...
	*anno = *((uint8_t *)item->exinf) != *((uint8_t *)src);
	switch(*(uint8_t *)src){
	case 0x41: tECNLDrEventController_cDrEventController_setFaultStatus_Fault( );
	case 0x42: tECNLDrEventController_cDrEventController_setFaultStatus_NoFault( );
		*((uint8_t *)item->exinf) = *((uint8_t *)src);
		break;
	default:
		return 0;
