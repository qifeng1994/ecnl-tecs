...
#include "gpio_api.h"
#include "rtc_api.h"
#include "tECNLBathHeatingStatusSensor_tecsgen.h"
...
struct bath_heating_status_sensor_t bath_heating_status_sensor_class_data = {
};
...
int onoff_prop_set(const EPRPINIB *item, const void *src, int size, bool_t *anno)
{
	...
	*anno = *((uint8_t*)item->exinf) != *((uint8_t*)src);
	switch(*(uint8_t *)src){
	case 0x30: tECNLBathHeatingStatusSensor_cBathHeatingStatusSensor_setOperatingStatus_ON( );
		break;
	case 0x31: tECNLBathHeatingStatusSensor_cBathHeatingStatusSensor_setOperatingStatus_OFF( );
		break;
	default:
		return 0;
	}return 1;
...
int alarm_prop_set(const EPRPINIB *item, const void *src, int size, bool_t *anno)
{
	...
	*anno = *((uint8_t *)item->exinf) != *((uint8_t *)src);
	switch(*(uint8_t *)src){
	case 0x41: tECNLBathHeatingStatusSensor_cBathHeatingStatusSensor_setFaultStatus_Fault( );
	case 0x42: tECNLBathHeatingStatusSensor_cBathHeatingStatusSensor_setFaultStatus_NoFault( );
