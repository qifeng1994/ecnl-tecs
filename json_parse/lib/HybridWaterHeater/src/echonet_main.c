...
struct hybrid_water_heater_t hybrid_water_heater_class_data = {
};
void linkage_mode_setting_for_solar_power_generation_prop_set (const EPRPINIB *item, const void *src, int size, bool_t *anno)
{
    ...
    *anno = *((uint8_t*)item->exinf) != *((uint8_t*)src);
    switch (*(uint8_t*)src) {
      case 0x41: cHybridWaterHeater_setModeOff( );
        break;
      case 0x42: cHybridWaterHeater_setHouseholdConsumption( );
        break;
      case 0x43: cHybridWaterHeater_setPrioritizingElectricitySales( );
        break;
      case 0x44: cHybridWaterHeater_setEconomicEfficiency( );
        break;
default:
        return 0;
}
