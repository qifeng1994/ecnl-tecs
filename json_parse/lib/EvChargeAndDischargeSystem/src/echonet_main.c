...
#include "gpio_api.h"
#include "rtc_api.h"
#include "tECNLEvChargeAndDischargeSystem_tecsgen.h"
...
struct ev_charge_and_discharge_system_t ev_charge_and_discharge_system_class_data = {
};
...
int onoff_prop_set(const EPRPINIB *item, const void *src, int size, bool_t *anno)
{
	...
	*anno = *((uint8_t*)item->exinf) != *((uint8_t*)src);
	switch(*(uint8_t *)src){
	case 0x30: tECNLEvChargeAndDischargeSystem_cEvChargeAndDischargeSystem_setOperatingStatus_ON( );
		break;
	case 0x31: tECNLEvChargeAndDischargeSystem_cEvChargeAndDischargeSystem_setOperatingStatus_OFF( );
		break;
	default:
		return 0;
	}return 1;
...
int alarm_prop_set(const EPRPINIB *item, const void *src, int size, bool_t *anno)
{
	...
	*anno = *((uint8_t *)item->exinf) != *((uint8_t *)src);
	switch(*(uint8_t *)src){
	case 0x41: tECNLEvChargeAndDischargeSystem_cEvChargeAndDischargeSystem_setFaultStatus_Fault( );
	case 0x42: tECNLEvChargeAndDischargeSystem_cEvChargeAndDischargeSystem_setFaultStatus_NoFault( );
