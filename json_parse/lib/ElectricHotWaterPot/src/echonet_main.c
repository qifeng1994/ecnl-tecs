...
#include "gpio_api.h"
#include "rtc_api.h"
#include "tECNLElectricHotWaterPot_tecsgen.h"
...
struct electric_hot_water_pot_t electric_hot_water_pot_class_data = {
};
...
int onoff_prop_set(const EPRPINIB *item, const void *src, int size, bool_t *anno)
{
	...
	*anno = *((uint8_t*)item->exinf) != *((uint8_t*)src);
	switch(*(uint8_t *)src){
	case 0x30: tECNLElectricHotWaterPot_cElectricHotWaterPot_setOperatingStatus_ON( );
		break;
	case 0x31: tECNLElectricHotWaterPot_cElectricHotWaterPot_setOperatingStatus_OFF( );
		break;
	default:
		return 0;
	}return 1;
...
int alarm_prop_set(const EPRPINIB *item, const void *src, int size, bool_t *anno)
{
	...
	*anno = *((uint8_t *)item->exinf) != *((uint8_t *)src);
	switch(*(uint8_t *)src){
	case 0x41: tECNLElectricHotWaterPot_cElectricHotWaterPot_setFaultStatus_Fault( );
	case 0x42: tECNLElectricHotWaterPot_cElectricHotWaterPot_setFaultStatus_NoFault( );
