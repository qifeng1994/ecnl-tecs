...
#include "gpio_api.h"
#include "rtc_api.h"
#include "tECNLTemperatureSensor_tecsgen.h"
...
struct temperature_sensor_t temperature_sensor_class_data = {
};
...
int onoff_prop_set(const EPRPINIB *item, const void *src, int size, bool_t *anno)
{
	...
	*anno = *((uint8_t*)item->exinf) != *((uint8_t*)src);
	switch(*(uint8_t *)src){
	case 0x30: tECNLTemperatureSensor_cTemperatureSensor_setOperatingStatus_ON( );
		break;
	case 0x31: tECNLTemperatureSensor_cTemperatureSensor_setOperatingStatus_OFF( );
		break;
	default:
		return 0;
	}return 1;
...
int alarm_prop_set(const EPRPINIB *item, const void *src, int size, bool_t *anno)
{
	...
	*anno = *((uint8_t *)item->exinf) != *((uint8_t *)src);
	switch(*(uint8_t *)src){
	case 0x41: tECNLTemperatureSensor_cTemperatureSensor_setFaultStatus_Fault( );
	case 0x42: tECNLTemperatureSensor_cTemperatureSensor_setFaultStatus_NoFault( );
