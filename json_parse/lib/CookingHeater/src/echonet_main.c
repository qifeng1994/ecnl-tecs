...
#include "gpio_api.h"
#include "rtc_api.h"
#include "tECNLCookingHeater_tecsgen.h"
...
struct cooking_heater_t cooking_heater_class_data = {
};
struct node_profile_object_t local_node_data = {
};
int all_stop_setting_prop_set (const EPRPINIB *item, const void *src, int size, bool_t *anno)
{
    ...
    *anno = *((uint8_t*)item->exinf) != *((uint8_t*)src);
    switch (*(uint8_t*)src) {
      case 0x40: tECNLCookingHeater_cCookingHeater_setAllStopSetting_StopAllHeating( );
        break;
default:
        return 0;
	}return 1;
...
int onoff_prop_set(const EPRPINIB *item, const void *src, int size, bool_t *anno)
{
	...
	*anno = *((uint8_t*)item->exinf) != *((uint8_t*)src);
	switch(*(uint8_t *)src){
	case 0x30: tECNLCookingHeater_cCookingHeater_setOperatingStatus_ON( );
		break;
	case 0x31: tECNLCookingHeater_cCookingHeater_setOperatingStatus_OFF( );
		break;
	default:
		return 0;
	}return 1;
...
int alarm_prop_set(const EPRPINIB *item, const void *src, int size, bool_t *anno)
{
	...
	*anno = *((uint8_t *)item->exinf) != *((uint8_t *)src);
	switch(*(uint8_t *)src){
	case 0x41: tECNLCookingHeater_cCookingHeater_setFaultStatus_Fault( );
	case 0x42: tECNLCookingHeater_cCookingHeater_setFaultStatus_NoFault( );
