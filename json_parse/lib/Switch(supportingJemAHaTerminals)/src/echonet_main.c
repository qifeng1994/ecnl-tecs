...
#include "gpio_api.h"
#include "rtc_api.h"
#include "tECNLSwitch(supportingJemAHaTerminals)_tecsgen.h"
...
struct switch_(supporting_jem-a/ha_terminals)_t switch_(supporting_jem-a/ha_terminals)_class_data = {
};
...
int onoff_prop_set(const EPRPINIB *item, const void *src, int size, bool_t *anno)
{
	...
	*anno = *((uint8_t*)item->exinf) != *((uint8_t*)src);
	switch(*(uint8_t *)src){
	case 0x30: tECNLSwitch(supportingJemAHaTerminals)_cSwitch(supportingJemAHaTerminals)_setOperatingStatus_ON( );
		break;
	case 0x31: tECNLSwitch(supportingJemAHaTerminals)_cSwitch(supportingJemAHaTerminals)_setOperatingStatus_OFF( );
		break;
	default:
		return 0;
	}return 1;
...
int alarm_prop_set(const EPRPINIB *item, const void *src, int size, bool_t *anno)
{
	...
	*anno = *((uint8_t *)item->exinf) != *((uint8_t *)src);
	switch(*(uint8_t *)src){
	case 0x41: tECNLSwitch(supportingJemAHaTerminals)_cSwitch(supportingJemAHaTerminals)_setFaultStatus_Fault( );
	case 0x42: tECNLSwitch(supportingJemAHaTerminals)_cSwitch(supportingJemAHaTerminals)_setFaultStatus_NoFault( );
