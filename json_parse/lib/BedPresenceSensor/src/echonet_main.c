...
#include "gpio_api.h"
#include "rtc_api.h"
#include "tECNLBedPresenceSensor_tecsgen.h"
...
struct bed_presence_sensor_t bed_presence_sensor_class_data = {
};
...
int onoff_prop_set(const EPRPINIB *item, const void *src, int size, bool_t *anno)
{
	...
	*anno = *((uint8_t*)item->exinf) != *((uint8_t*)src);
	switch(*(uint8_t *)src){
	case 0x30: tECNLBedPresenceSensor_cBedPresenceSensor_setOperatingStatus_ON( );
		break;
	case 0x31: tECNLBedPresenceSensor_cBedPresenceSensor_setOperatingStatus_OFF( );
		break;
	default:
		return 0;
	}return 1;
...
int alarm_prop_set(const EPRPINIB *item, const void *src, int size, bool_t *anno)
{
	...
	*anno = *((uint8_t *)item->exinf) != *((uint8_t *)src);
	switch(*(uint8_t *)src){
	case 0x41: tECNLBedPresenceSensor_cBedPresenceSensor_setFaultStatus_Fault( );
	case 0x42: tECNLBedPresenceSensor_cBedPresenceSensor_setFaultStatus_NoFault( );
