...
#include "gpio_api.h"
#include "rtc_api.h"
#include "tECNLFanHeater_tecsgen.h"
...
struct fan_heater_t fan_heater_class_data = {
};
struct node_profile_object_t local_node_data = {
};
int temperatureSetValue_prop_set (const EPRPINIB *item, const void *src, int size, bool_t *anno)
{
    ...
    if((*(uint8_t*)src >= 0) && (*(uint8_t*)src <= 50)){
        *((uint8_t*)item->exinf) != *((uint8_t*)src);
        cFanHeater_SetTemperatureSetValue( );
    }
    else{
        return 0;
    }return 1;
...
int onoff_prop_set(const EPRPINIB *item, const void *src, int size, bool_t *anno)
{
	...
	*anno = *((uint8_t*)item->exinf) != *((uint8_t*)src);
	switch(*(uint8_t *)src){
	case 0x30: tECNLFanHeater_cFanHeater_setOperatingStatus_ON( );
		break;
	case 0x31: tECNLFanHeater_cFanHeater_setOperatingStatus_OFF( );
		break;
	default:
		return 0;
	}return 1;
...
int alarm_prop_set(const EPRPINIB *item, const void *src, int size, bool_t *anno)
{
	...
	*anno = *((uint8_t *)item->exinf) != *((uint8_t *)src);
	switch(*(uint8_t *)src){
	case 0x41: tECNLFanHeater_cFanHeater_setFaultStatus_Fault( );
	case 0x42: tECNLFanHeater_cFanHeater_setFaultStatus_NoFault( );
		*((uint8_t *)item->exinf) = *((uint8_t *)src);
		break;
	default:
		return 0;
