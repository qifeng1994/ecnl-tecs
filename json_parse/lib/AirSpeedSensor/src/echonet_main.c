...
#include "gpio_api.h"
#include "rtc_api.h"
#include "tECNLAirSpeedSensor_tecsgen.h"
...
struct air_speed_sensor_t air_speed_sensor_class_data = {
};
...
int onoff_prop_set(const EPRPINIB *item, const void *src, int size, bool_t *anno)
{
	...
	*anno = *((uint8_t*)item->exinf) != *((uint8_t*)src);
	switch(*(uint8_t *)src){
	case 0x30: tECNLAirSpeedSensor_cAirSpeedSensor_setOperatingStatus_ON( );
		break;
	case 0x31: tECNLAirSpeedSensor_cAirSpeedSensor_setOperatingStatus_OFF( );
		break;
	default:
		return 0;
	}return 1;
...
int alarm_prop_set(const EPRPINIB *item, const void *src, int size, bool_t *anno)
{
	...
	*anno = *((uint8_t *)item->exinf) != *((uint8_t *)src);
	switch(*(uint8_t *)src){
	case 0x41: tECNLAirSpeedSensor_cAirSpeedSensor_setFaultStatus_Fault( );
	case 0x42: tECNLAirSpeedSensor_cAirSpeedSensor_setFaultStatus_NoFault( );
