...
#include "gpio_api.h"
#include "rtc_api.h"
#include "tECNLParallelProcessingCombinationTypePowerControl_tecsgen.h"
...
struct parallel_processing_combination-type_power_control_t parallel_processing_combination-type_power_control_class_data = {
};
...
int onoff_prop_set(const EPRPINIB *item, const void *src, int size, bool_t *anno)
{
	...
	*anno = *((uint8_t*)item->exinf) != *((uint8_t*)src);
	switch(*(uint8_t *)src){
	case 0x30: tECNLParallelProcessingCombinationTypePowerControl_cParallelProcessingCombinationTypePowerControl_setOperatingStatus_ON( );
		break;
	case 0x31: tECNLParallelProcessingCombinationTypePowerControl_cParallelProcessingCombinationTypePowerControl_setOperatingStatus_OFF( );
		break;
	default:
		return 0;
	}return 1;
...
int alarm_prop_set(const EPRPINIB *item, const void *src, int size, bool_t *anno)
{
	...
	*anno = *((uint8_t *)item->exinf) != *((uint8_t *)src);
	switch(*(uint8_t *)src){
	case 0x41: tECNLParallelProcessingCombinationTypePowerControl_cParallelProcessingCombinationTypePowerControl_setFaultStatus_Fault( );
	case 0x42: tECNLParallelProcessingCombinationTypePowerControl_cParallelProcessingCombinationTypePowerControl_setFaultStatus_NoFault( );
