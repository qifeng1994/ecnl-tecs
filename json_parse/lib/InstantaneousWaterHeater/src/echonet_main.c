...
#include "gpio_api.h"
#include "rtc_api.h"
#include "tECNLInstantaneousWaterHeater_tecsgen.h"
...
struct instantaneous_water_heater_t instantaneous_water_heater_class_data = {
};
struct node_profile_object_t local_node_data = {
};
int bath_auto_mode_setting_prop_set (const EPRPINIB *item, const void *src, int size, bool_t *anno)
{
    ...
    *anno = *((uint8_t*)item->exinf) != *((uint8_t*)src);
    switch (*(uint8_t*)src) {
      case 0x41: tECNLInstantaneousWaterHeater_cInstantaneousWaterHeater_setBathAutoModeSetting_On( );
        break;
      case 0x42: tECNLInstantaneousWaterHeater_cInstantaneousWaterHeater_setBathAutoModeSetting_Off( );
        break;
default:
        return 0;
	}return 1;
...
int onoff_prop_set(const EPRPINIB *item, const void *src, int size, bool_t *anno)
{
	...
	*anno = *((uint8_t*)item->exinf) != *((uint8_t*)src);
	switch(*(uint8_t *)src){
	case 0x30: tECNLInstantaneousWaterHeater_cInstantaneousWaterHeater_setOperatingStatus_ON( );
		break;
	case 0x31: tECNLInstantaneousWaterHeater_cInstantaneousWaterHeater_setOperatingStatus_OFF( );
		break;
	default:
		return 0;
	}return 1;
...
int alarm_prop_set(const EPRPINIB *item, const void *src, int size, bool_t *anno)
{
	...
	*anno = *((uint8_t *)item->exinf) != *((uint8_t *)src);
	switch(*(uint8_t *)src){
	case 0x41: tECNLInstantaneousWaterHeater_cInstantaneousWaterHeater_setFaultStatus_Fault( );
	case 0x42: tECNLInstantaneousWaterHeater_cInstantaneousWaterHeater_setFaultStatus_NoFault( );
