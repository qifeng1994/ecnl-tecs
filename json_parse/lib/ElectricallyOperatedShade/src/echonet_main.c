...
struct electrically_operated_shade_t electrically_operated_shade_class_data = {
};
void open/close_setting_prop_set (const EPRPINIB *item, const void *src, int size, bool_t *anno)
{
    ...
    *anno = *((uint8_t*)item->exinf) != *((uint8_t*)src);
    switch (*(uint8_t*)src) {
      case 0x41: cElectricallyOperatedShade_setOpen( );
        break;
      case 0x42: cElectricallyOperatedShade_setClose( );
        break;
default:
        return 0;
}
