...
struct audio_t audio_class_data = {
};
...
int onoff_prop_set(const EPRPINIB *item, const void *src, int size, bool_t *anno)
{
	...
	*anno = *((uint8_t*)item->exinf) != *((uint8_t*)src);
	switch(*(uint8_t *)src){
	case 0x30: tECNLAudio_cAudio_setOperatingStatus_ON( );
		break;
	case 0x31: tECNLAudio_cAudio_setOperatingStatus_OFF( );
		break;
	default:
		return 0;
	}return 1;
...
int alarm_prop_set(const EPRPINIB *item, const void *src, int size, bool_t *anno)
{
	...
	*anno = *((uint8_t *)item->exinf) != *((uint8_t *)src);
	switch(*(uint8_t *)src){
	case 0x41: tECNLAudio_cAudio_setFaultStatus_Fault( );
	case 0x42: tECNLAudio_cAudio_setFaultStatus_NoFault( );
