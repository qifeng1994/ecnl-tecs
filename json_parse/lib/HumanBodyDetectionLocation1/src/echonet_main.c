...
#include "gpio_api.h"
#include "rtc_api.h"
#include "tECNLHumanBodyDetectionLocation1_tecsgen.h"
...
struct human_body_detection_location_1_t human_body_detection_location_1_class_data = {
};
...
int onoff_prop_set(const EPRPINIB *item, const void *src, int size, bool_t *anno)
{
	...
	*anno = *((uint8_t*)item->exinf) != *((uint8_t*)src);
	switch(*(uint8_t *)src){
	case 0x30: tECNLHumanBodyDetectionLocation1_cHumanBodyDetectionLocation1_setOperatingStatus_ON( );
		break;
	case 0x31: tECNLHumanBodyDetectionLocation1_cHumanBodyDetectionLocation1_setOperatingStatus_OFF( );
		break;
	default:
		return 0;
	}return 1;
...
int alarm_prop_set(const EPRPINIB *item, const void *src, int size, bool_t *anno)
{
	...
	*anno = *((uint8_t *)item->exinf) != *((uint8_t *)src);
	switch(*(uint8_t *)src){
	case 0x41: tECNLHumanBodyDetectionLocation1_cHumanBodyDetectionLocation1_setFaultStatus_Fault( );
	case 0x42: tECNLHumanBodyDetectionLocation1_cHumanBodyDetectionLocation1_setFaultStatus_NoFault( );
