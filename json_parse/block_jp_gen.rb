...
def parse_properties (val,options)
    val.each{ |prop_id,val2|
        if val2['oneOf'] then
                ...
                propertyName_ja = val2['propertyName']['ja']
                if val2['data']['$ref'] then
                ref = val2['data']['$ref'].sub( /#\/definitions\//, "" )
                val3 = Definitions[ref]
                parse_definitions(val3,propertyName,options,propertyName_ja)
                ...
                val2['data']['oneOf'].each{ |val3|
                    ...
                    parse_definitions(val4,propertyName,options,propertyName_ja)
                    else
                    end
                }
                else
...
def parse_definitions(val,propertyName,options,propertyName_ja)
    if val['type'] == 'number' then
        ...
        propertyName = font_change(propertyName)
        print_array(propertyName,val,options,propertyName_ja)
    else
    end

end

def print_array(propertyName,val,options,propertyName_ja)

    val['enum'].each{|edt|
        option = Array.new
        option.push("#{propertyName_ja}  #{edt['state']['ja']}")
...
def temp_method (val,array)
    ...
    className_ja = val['className']['ja']
    blocks = Hash.new
    args0 = []
    options =[["動作状態 入","OperatingStatus  ON"],["動作状態 切","OperatingStatus  OFF"]]
    
    blocks = {"type" => "#{className}", "message0" => "#{className_ja} %1", "output" => nil, "colour" => 230}
...
blockly = File.open("block_jp.json","w+")
blockly.puts json
blockly.close
