...
Code.initLanguage = function() {
 ...
 Code.runJS =function(ipData, freeData) {
    ...
    //ipData='192.168.137.112';
    let uint8Array = [];
    let binaryString = "";

    //todo:按照一定长度切割字符串，每次提取一段字符串，让系统判断ipData
    freeData = Blockly.JavaScript.workspaceToCode(Code.workspace);
    for(var i=0,len=freeData.length/44;i<len;i++){
    //console.log('freeData ='+freeData);
    //利用analyzeFreeData函数，分析freeData的deoj
    var freeData1 = freeData.substr(0,44);
    ...
    freeData = freeData.replace(freeData1,'');
  }
}
//把freeData转换成数组？
//ssng.js里面有相同的函数，所以优先用了ssng.js的函数
...
//定义analyzeFreeData函数
//提取eoj的字符串，如果是0290那么ipData是112，如果是0130那么ipData是114
