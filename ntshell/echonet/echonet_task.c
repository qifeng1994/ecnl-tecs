...
void echonet_task(intptr_t fa_exinf)
{
	...
	//syslog(LOG_NOTICE,"initialize over");
	...
	const EOBJINIB *p_sobj, ATR sobjatr)
{
	...
	case ESV_SET_I:
		...
		_ecn_tsk_eoj_set(p_obj, eobjatr, fromapp, false, fa_fbs_id, ESV_NOP, ESV_SET_I_SNA, &a_fbs_anno);
			/* 0; 0x50 */
		result = 0;
		break;
	...
	int *p_sw_ok)
{
	...
	for (i = 0; i < a_count; i++) {
		...
		if ((a_eprp != NULL)
			...
			a_update = a_anno;
			syslog(LOG_NOTICE,"set edt");
			a_ans = _ecn_tsk_eoj_set_edt(a_eprp, p_edt->payload, a_size, &a_update);
			//syslog(LOG_NOTICE,"%d",a_ans);
			syslog(LOG_NOTICE,"set edt over");
			if (a_anno && (a_ans > 0))
				a_anno = a_update;
		}
		else {
	...
	ECN_FBS_ID *fa_fbs_anno)
{
	...
	/* プロパティ値書き込み実行 */
	syslog(LOG_NOTICE,"property write");
	...
	syslog(LOG_NOTICE,"property write over");
	if (a_ret)
		goto lb_except;
