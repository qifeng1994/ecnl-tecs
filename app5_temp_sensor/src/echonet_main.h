...
#define EOJ_X3_TEMP_SENSOR_02 2	/* 温度センサクラスのインスタンスコード */

#define EOJ_X3_HUMIDITY_SENSOR 1	/* 湿度センサクラスのインスタンスコード */
...
struct ecn_cls0EF0_prp82_t {
	/* メジャーバージョン(小数点以上) */
	uint8_t field03770028;
	/* マイナーバージョン(小数点以下) */
	uint8_t field032F0169;
	/* 電文タイプ */
	uint8_t field00A70CB4[2];
...
struct ecn_cls0EF0_prp83_t {
	/* 下位通信層IDフィールド */
	uint8_t field01DF725B;
	/* メーカーコード */
	struct field0073673B_t field0073673B;
	/* ユニークID部(メーカー独自) */
	uint8_t field00DB0535[13];
} __attribute__((packed));
...
extern struct ecn_cls0011_t temperature_sensor1_data;	/* 温度センサクラスのデータ */
extern struct ecn_cls0011_t temperature_sensor2_data;	/* 温度センサクラスのデータ */
extern struct ecn_cls0012_t humidity_sensor_data;	/* 湿度センサクラスのデータ */

EPRP_SETTER onoff_prop_set;
EPRP_SETTER alarm_prop_set;
EPRP_SETTER temperature_set;
EPRP_GETTER temperature_get;
